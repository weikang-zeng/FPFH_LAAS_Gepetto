...
namespace multiscale_fpfh
{
    ...
    void MFPFHLocalizer::runClusters(){
        ...
        /*std::cout<<"\n\e[1mStarting comparison\e[0m\n\n";
        fileOut<<"\n\e[1mStarting comparison\e[0m\n\n";

        std::vector<ResultCompareT> vecResults;
        float tempFitness = std::numeric_limits<float>::max();
        Eigen::Isometry3d tempPose = Eigen::Isometry3d::Identity();
        for(size_t indexCluster = 0; indexCluster < clustersPoints_.size(); indexCluster++){
            for (size_t indexView = 0; indexView < modelObject_.size(); indexView++){
                // tempFitness = compare2Scene(modelObject_[indexView].points, modelObject_[indexView].scales[5].features, tempPose);
                tempFitness = compare2Cluster(modelObject_[indexView].points, modelObject_[indexView].scales[5].features, indexCluster, tempPose);
                ResultCompareT tempresult;
                // tempresult.indexCluster = -1;
                tempresult.indexCluster = indexCluster;
                tempresult.indexInModel = indexView;
                tempresult.fitness = tempFitness;
                tempresult.pose = tempPose;
                vecResults.push_back(tempresult);
                // std::cout<<"For cloud \""<<modelObject_[indexView].partName<<"\":"<<std::endl<<tempresult;
                // fileOut<<"For cloud \""<<modelObject_[indexView].partName<<"\":"<<std::endl<<tempresult;
                std::cout<<"For cloud \""<<modelObject_[indexView].partName<<"\" and cluster ["<<indexCluster<<"]:"<<std::endl<<tempresult;
                fileOut<<"For cloud \""<<modelObject_[indexView].partName<<"\" and cluster ["<<indexCluster<<"]:"<<std::endl<<tempresult;
            }
        }*/
        //std::sort(vecResults.begin(), vecResults.end()/*, MFPFHLocalizer::compareComparisonResults*/);

        /*std::cout<<"\n\n\e[1mFirst result:\e[0m \n"<<vecResults.front()<<std::endl;
        std::cout<<"\n\n\e[1mLast result:\e[0m \n"<<vecResults.back()<<std::endl;
        fileOut<<"\n\n\e[1mFirst result:\e[0m \n"<<vecResults.front()<<std::endl;
        fileOut<<"\n\n\e[1mLast result:\e[0m \n"<<vecResults.back()<<std::endl;

        pcl::visualization::PCLVisualizer viewer("Results (front/back)");
        int vpLeft, vpRight;
        viewer.createViewPort(0., 0., 0.5, 1., vpLeft);
        viewer.addText("Front Result", 10, 10, "leftLabel", vpLeft);
        viewer.createViewPort(0.5, 0., 1., 1., vpRight);
        viewer.addText("Back Result", 10, 10, "rightLabel", vpRight);
        viewer.setBackgroundColor(0.6, 0.6, 0.6);
        viewer.addCoordinateSystem(0.01);

        CloudT::Ptr frontCloud(new CloudT), backCloud(new CloudT);
        Eigen::Matrix4f frontTransform(vecResults.front().pose.cast<float>().matrix());
        Eigen::Matrix4f backTransform(vecResults.back().pose.cast<float>().matrix());
        pcl::transformPointCloud(*(modelObject_[vecResults.front().indexInModel].points), *frontCloud, frontTransform);
        pcl::transformPointCloud(*(modelObject_[vecResults.back().indexInModel].points), *backCloud, backTransform);
        viewer.addPointCloud(scenePoints_, "scene");
        // viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, 0.5, 0.5, 0.5, "scene");
        viewer.addPointCloud(frontCloud, "frontResult", vpLeft);
        viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, 1., 0., 0., "frontResult");
        viewer.addPointCloud(clustersPoints_[vecResults.front().indexCluster], "frontCluster", vpLeft);
        viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, 0., 0., 1., "frontCluster");
        viewer.addPointCloud(backCloud, "backResult", vpRight);
        viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, 0., 1., 0., "backResult");
        viewer.addPointCloud(clustersPoints_[vecResults.back().indexCluster], "backCluster", vpRight);
        viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, 0., 0., 1., "backCluster");
